...
public class PPListener implements Listener {
	...
	public void onChat(AsyncPlayerChatEvent e) {
		...
		if (inst.downloadFile.containsKey(e.getPlayer().getUniqueId())) {
			...
			File outputfile = new File(inst.images
					+ File.separator
					+ inst.downloadFile.get(e.getPlayer().getUniqueId())
							.getName()
			//		+ inst.downloadFile.get(e.getPlayer().getUniqueId())
				//			.getType()
							+(inst.downloadFile.get(e.getPlayer().getUniqueId()).getType()
									.equalsIgnoreCase(".txt")? ".txt": (e.getMessage().endsWith("gif")?".gif":".jpg")));
			...
			if (inst.downloadFile.get(e.getPlayer().getUniqueId()).getType()
